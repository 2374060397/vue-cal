<template>
  <div class="toggler" :style="{ ...buttonSizeStyles }">
    <label>
      <input type="checkbox" :checked="checked" @input="updateHandler"> <!-- @input为v-on的指令集，监听我的input，当有事件的时候，触发后面命名的函数 -->
      <span></span>
    </label>
    <strong>
      <slot />
...
<script>
export default {
    //props 当前再子组件对吧，但我去看下父亲组件，props的属性就在那可以用
  props: {
    size: {
      type: Number,
      default: 1,
  ...
  methods: {
    updateHandler(e) {
      ...
      this.$emit('updated', e.target.checked) //$emit 触发当前实例上的事件。附加参数都会传给监听器回调。
    }
  }
}
</script>
